...
module idf.driver.spi_master;

import idf.common.esp_err;
import idf.hal.spi_types;
import idf.soc.soc;

/// 8 Mhz
enum SPI_MASTER_FREQ_8M = APB_CLK_FREQ / 10;
/// 8.89 MHz
enum SPI_MASTER_FREQ_9M = APB_CLK_FREQ / 9;
/// 10 Mhz
enum SPI_MASTER_FREQ_10M = APB_CLK_FREQ / 8;
/// 11.43 MHz
enum SPI_MASTER_FREQ_11M = APB_CLK_FREQ / 7;
/// 13.33 MHz
enum SPI_MASTER_FREQ_13M = APB_CLK_FREQ / 6;
/// 16 MHz
enum SPI_MASTER_FREQ_16M = APB_CLK_FREQ / 5;
/// 20 MHz
enum SPI_MASTER_FREQ_20M = APB_CLK_FREQ / 4;
/// 26.67 MHz
enum SPI_MASTER_FREQ_26M = APB_CLK_FREQ / 3;
/// 40 MHz
enum SPI_MASTER_FREQ_40M = APB_CLK_FREQ / 2;
/// 80 MHz
enum SPI_MASTER_FREQ_80M = APB_CLK_FREQ / 1;

/// Transmit command/address/data LSB first instead of the default MSB first.
enum SPI_DEVICE_TXBIT_LSBFIRST = 1 << 0;
/// Receive data LSB first instead of the default MSB first.
enum SPI_DEVICE_RXBIT_LSBFIRST = 1 << 1;
/// Transmit and receive LSB first.
enum SPI_DEVICE_BIT_LSBFIRST = SPI_DEVICE_RXBIT_LSBFIRST | SPI_DEVICE_TXBIT_LSBFIRST;
/// Use MOSI (=spid) for both sending and receiving data.
enum SPI_DEVICE_3WIRE = 1 << 2;
/// Make CS positive during a transaction instead of negative.
enum SPI_DEVICE_POSITIVE_CS = 1 << 3;
/// Transmit data before receiving it, instead of simultaneously.
enum SPI_DEVICE_HALFDUPLEX = 1 << 4;
/// Output clock on CS line if CS is active.
enum SPI_DEVICE_CLK_AS_CS = 1 << 5;
/**
...
*/
enum SPI_DEVICE_NO_DUMMY = 1 << 6;
enum SPI_DEVICE_DDRCLK = 1 << 7;

alias transaction_cb_t = void function(spi_transaction_t* transaction);
...
struct spi_device_interface_config_t
{
	...
	*/
	...
	ushort cs_ena_pretrans;
    /// Amount of SPI bit-cycles the cs should stay active after the transmission (0-16)
	ubyte cs_ena_posttrans;
	...
	*/
	...
	This callback is called within interrupt context should be in IRAM for best
	performance, see "Transferring Speed" section in the SPI Master
	documentation for full details. If not, the callback may crash during flash
	operation when the driver is initialized with ESP_INTR_FLAG_IRAM.
	*/
...
struct spi_transaction_t
{
    ...
    */
	...
	/// Total data length received, should not be greater than `length` in
...
Params:
	handle = Device handle obtained using spi_host_add_dev
	trans_desc = Pointer to variable able to contain a pointer to the description of the transaction
		that is executed. The descriptor should not be modified until the descriptor is returned by
		spi_device_get_trans_result.
	ticks_to_wait = Ticks to wait until there's a returned item; use port
		`MAX_DELAY` to never time out.
Returns:
 - ESP_ERR_INVALID_ARG   if parameter is invalid
...
This function is the equivalent of calling spi_device_queue_trans() followed by
spi_device_get_trans_result(). Do not use this when there is still a transaction
separately queued (started) from spi_device_queue_trans() or
polling_start/transmit that hasn't been finalized.

Notes: This function is not thread safe when multiple tasks access the same SPI
	device. Normally a device cannot start (queue) polling and interrupt
...
Note: Normally a device cannot start (queue) polling and interrupt transactions
	simutanuously. Moreover, a device cannot start a new polling transaction if
	another polling transaction is not finished.

Params:
	handle = Device handle obtained using spi_host_add_dev
	trans_desc = Description of transaction to execute
	ticks_to_wait = Ticks to wait until there's room in the queue; currently
		only port `MAX_DELAY` is supported.
...
Returns:
 ...
 - ESP_OK                on success
*/
esp_err_t spi_device_polling_start(
	spi_device_handle_t handle,
	spi_transaction_t* trans_desc,
	TickType_t ticks_to_wait
);
...
Returns:
 ...
 - ESP_OK                on success
*/
esp_err_t spi_device_polling_end(
	spi_device_handle_t handle,
	TickType_t ticks_to_wait
);
...
Notes: The function will wait until all the existing transactions have been sent.
...
Returns:
 - ESP_ERR_INVALID_ARG : `wait` is not set to port `MAX_DELAY`.
 - ESP_OK : Success.
*/
...
*/
void spi_device_release_bus(
	spi_device_handle_t dev
);

/**
Calculate the working frequency that is most close to desired frequency, and
also the register value.
...
Notes: If **dummy_o** is not zero, it means dummy bits should be applied in half
	duplex mode, and full duplex mode may not work.
*/
void spi_get_timing(
	bool gpio_is_used,
	int input_delay_ns,
